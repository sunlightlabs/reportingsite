    ...
    def handle(self, *args, **options):
        ...
        for candidate_id in candidate_ids:
            ...
            if (candidate_id['candidate__fec_id']):
                ...
                candidate = Candidate_Overlay.objects.get(fec_id=candidate_id['candidate__fec_id'])
                total_supporting = Expenditure.objects.filter(superceded_by_amendment=False, candidate=candidate, support_oppose__iexact='S').aggregate(total=Sum('expenditure_amount'))
                total_opposing = Expenditure.objects.filter(superceded_by_amendment=False, candidate=candidate, support_oppose__iexact='O').aggregate(total=Sum('expenditure_amount'))
                total = Expenditure.objects.filter(superceded_by_amendment=False, candidate=candidate).aggregate(total=Sum('expenditure_amount'))
