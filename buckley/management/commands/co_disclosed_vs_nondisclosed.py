...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for race in races:
            non_discloser_ids = Expenditure.objects.filter(race=race, committee__has_donors=False).order_by('committee').values_list('committee', flat=True)
            ...
            discloser_ids = Expenditure.objects.filter(race=race, committee__has_donors=True).order_by('committee').values_list('committee', flat=True)
