...
class Organization(models.Model):
    ...
    def __unicode__(self):
        return self.crp_name or self.name
    ...
    def issue_counts(self):
        issues = defaultdict(int)
        for issue in IssueCode.objects.all():
            count = issue.registration_set.filter(organization=self).count()
            if not count:
                continue
            issues[issue] = count
        return sorted(issues.items(), key=itemgetter(1), reverse=True)

    def issue_counts_for_org_slug(self):
        """Because there can be (and are) several organizations
        that have the same slug (because of name standardization),
        this method gives a list of issues and counts for organizations
        with the same slug as this one.
        """
        return IssueCode.objects.filter(registration__organization__slug=self.slug).annotate(num=Count('registration')).filter(num__gt=0).order_by('-num')
    ...
    def slugify(self):
        if self.crp_name:
            return slugify(self.crp_name)[:50]
...
class Registration(models.Model):
    ...
    def upload_to_scribd(self):
        ...
        pdf = pyPdf.PdfFileReader(StringIO(urllib2.urlopen(self.house_pdf_url()).read()))
        try:
            pdf.decrypt(r'') # Encrypted with a blank password
        except KeyError:
            pass
        ...
        description = 'This registration was filed with the House'
        if self.signed_date:
            description += ' on %s' % self.signed_date.strftime('%B %d, %Y')
        else:
            description += ' in %s' % self.report_year
